...
class AnomalyMapGenerator:
    ...
    def compute_anomaly_map(
        ...
        for layer_idx in range(len(self.pool_layers)):
            # pylint: disable=not-callable, this is no recommended
            # test_norm = torch.tensor(distribution[layer_idx], dtype=torch.double)
            test_norm = torch.clone(distribution[layer_idx]).detach()
